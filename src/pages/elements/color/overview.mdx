...
Maintaining consistent and engaging digital interfaces throughout IBM, whether
applications or experiences, demands extended guidance around colour usage. The
...
<AnchorLink>Colour anatomy</AnchorLink>
<AnchorLink>Implementing colour</AnchorLink>
...
Application of the colour palette brings a unified and recognizable consistency
...
![6 screenshots showing a range of colour schemes and considerations for text and data visualizations](images/introduction.gif)
![UI colour alternatives](images/introduction.jpeg)

</GifPlayer>

## Colour anatomy

Carbon’s default themes are derived from the IBM Design Language colour palette.
...
The core green family serves as the primary action colour across all products
and experiences. Additional colours are used sparingly and purposefully.
...
<Caption>Alert Colours</Caption>

### Layering model

Colours in the neutral gray palette are layered on top of each other to create
depth and spatial associations. The layering model defines the logic of how
colours stack on top of each other in a UI when using the Carbon themes. Aspects
of the layering model are built directly into the themes, colour tokens, and
components.
...
Carbon uses tokens and themes to manage colour. Tokens are role-based, and themes
specify the colour values that serve those roles in the UI.

| Term    | Definition                                                                                                                                                                      |
| ------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| _Theme_ | A theme is a collection of colours designed to create a specific aesthetic. Themes control the colour value assigned to a token. For example, Gray 100 theme.                     |
| _Token_ | A token is the role-based identifier that assigns a colour. Unlike hex codes, tokens apply universally across themes. For example, `$layer`, `$border-subtle`, `$support-error`. |
| _Role_  | A role is the systematic usage of a colour assigned to a token. Roles cannot be changed between themes.                                                                          |
| _Value_ | A value is the unique visual attribute (hex code, rgba value) assigned to a token through the use of themes.                                                                    |

## Themes

Themes serve as an organizational framework for colour in Carbon, with each theme
based on a specific primary background colour. And they actually get their names
from their background colour. There are two default _light_ themes and two
...
universal colour tokens use the primary background colour as the base of its
layering model.
...
<br />
<Title> Global background colours </Title>
...
There are two light themes in Carbon: White and Gray 10. For enabled UI colours
light themes primarily use the colour range of White to Gray 20, and for text and
icons uses the colour range between Gray 100 and Gray 60.
...
- **White theme**: uses White as the global background colour and is layered
  first with components using Gray 10 backgrounds. The second layer uses White
  and the third layer used Gray 10.
- **Gray 10 theme:** uses Gray 10 as the global background colour and is layered
...
There are two dark themes: Gray 90 and Gray 100. For enabled UI colours, dark
themes primarily use the colour range of Gray 100 through Gray 70, and for text
and icons uses the colour range between White and Gray 50.
...
- **Gray 90 theme**: uses Gray 90 as the global background colour and is layered
  first with components using Gray 80 backgrounds. The second layer uses Gray 70
  and the third layer used Gray 60.
- **Gray 100 theme:** uses Gray 100 as the global background colour and is
...
Tokens are a method of applying colour in a consistent, reusable, and scalable
way. They help us abstract how we use colour from the values themselves. They are
...
as well as making possible colour functionalities like inline theming and light
or dark mode.

Each token is assigned a role and a value. The role determines what element to
apply a token too and the value is the actual colour (hex code) that appears in
the assigned theme. Colour token names and roles are the same across themes, only
...
See the [Tokens](/elements/color/tokens/) tab for the full list of colour tokens.
...
the general UI element the colour is being applied to,
...
<Caption>
  Colour tokens for components are the same across themes as shown by this text
...
Colour tokens that can be applied across components are called _core tokens_.
There are ten main groups of core colour tokens. They are grouped by the common
UI element that they are applied to. Token groups makes it easier to find and
apply colour tokens. Interaction state tokens are included in the group along
...
Some components have their own specific colour tokens, known as _component
...
The colour layering model for interaction tokens is as follows:
...
adjacent colours on the IBM core colour palette steps. These values fall outside
of the IBM core colour palette steps and have their own spectrum. Hover colours
...
will change to the `primary` colour on hover, giving them a subtle emphasis. Most
of the time, this shift in colour (on the text or icon element) will be
accompanied by a background hover colour shift as well. For example, an overflow
...
<Caption>
  Shown in the white theme, '$layer' enabled colour (1) has a value of 10 and
...
lighter or darker on the IBM colour scale. For example, the Blue 60 active state
is Blue 80.
...
<Caption>
  Shown in the White theme, `$button-primary` enabled colour (1) has a value of
  Blue 60 and `$button-primary-active` active colour (2) has a value of Blue 80.
</Caption>
...
`-selected` added after base token name, such as `$layer-selected-01`. The colour
logic for selected state is either one full step lighter or darker on the IBM
colour scale. For example, the Gray100 selected state is Gray 90.
...
Elements like text or icons that use `secondary` colors for their enabled state,
will change to the `primary` colour when selected, giving them a subtle emphasis.
Most of the time, this shift in colour (to the text or icon element) will be
accompanied by a selected background colour shift as well.
...
<Caption>
  Shown in the white theme, `$layer` enabled colour (1) has a value of Gray 10
  and `$layer-selected` selected colour (2) has a value of Gray 20.
</Caption>
...
to identify and locate on a page, most focus states use only one colour per theme
controlled through the `$focus` colour token.
...
The exception is high contrast moments where a `$focus-inverse` colour is used
instead.

Focus states are required on all interactive elements and must pass 3:1 colour
...
<Caption>
  White theme `$focus` colour (1) and Gray 90 theme `$focus` colour (2).
</Caption>
...
Disabled elements are always styled in the Gray family no matter its base colour.
...
- For the light themes, disabled colour values range from White to Gray 50
- For the dark themes, disabled colour values range from Gray 90 to Gray 40

## Accessibility

Using various forms of contrast is the most important consideration when making
user-friendly colour and interface choices. Awareness of standards and best
practices is the key to accessible colour selections.
...
The IBM palette is comprised of twelve colour grades—Black, White and ten values
for each hue. The following table indicates the minimum number of steps required
to achieve commonly used contrast ratios between any two colours.

<Row>
<Column colLg={8}>
<ArtDirection>

![colour grades - Mobile image](images/colorgrades_table_mobile.png)

![colour grades graphic](images/colorgrades_table.png)

</ArtDirection>
</Column>
</Row>

| Colour 1 | Colour 2 (4.5:1 contrast)   | Colour 2 (3:1 contrast)     |
| ------- | -------------------------- | -------------------------- |
...
<Column colLg={4} colMd={4} noGutterSm>
  <ResourceCard
    subTitle="RGB colour palettes (.ase and .clr)"
...
<Column colLg={4} colMd={4}  noGutterSm>
  <ResourceCard
    subTitle="Elements package: Colour"
