...
Implementing complex colour relationships can be challenging. Provided here are
some methods and techniques to help solve your product's colour and theming
needs.
...
components. They are used just like any other colour token, and in fact, are a
replacement for the `ui` colour tokens from v10. Layering tokens come in
...
colour (excluding interaction colours) is another layer and will require the use
of a different set of layering tokens.

For more information about how colour token migration works, see the
...
Not all colour tokens are part of a layer set. Some tokens groups, like `text`
and `icon`, work across layers. For a list of the layering tokens, see the colour
...
considered a proper layer but a supporting colour for `$layer` inside of
...
always built component colour variations in Carbon. For each layer that a
...
that are not part of the layer sets like type or icons, apply colour tokens as
...
Build a single component as you would normally but apply the contextual colour
...
Only use inline theming for major shifts in colour, like high contrast moments.
The more subtle transitions of colour in a product are handled within each theme
...
In order to implement inline theming, you must use the Carbon colour tokens and
...
an UI that is predominately light or dark in colour. The UI will automatically
switch from using light colours backgrounds with dark colour text to using dark
colour backgrounds with light colour text.
...
The themes use colour tokens to interpret which values are needed for each theme.
It is the colour tokens that allows a UI to so easily switch from one mode to the
next. You cannot implement light or dark mode without using colour tokens
...
You can build a light or dark mode by using the Carbon themes and colour tokens.
Your product will need to choose a light Carbon theme (White or Gray 10) and a
dark Carbon theme (Gray 100 or Gray 90).  All colour in your designs and
components whether Carbon, PAL or custom made should be redlined using the
Carbon colour tokens (it would also be to your advantage to use the colour token
...
changing colour between modes. Otherwise you may end up having very high contrast
...
![A dark UI featuring a light illustration with a white background](images/light-dark-mode-dont-1.png)
  ...
  <DoDont
    type="do"
    colLg={6}
    caption="If only one image is available between modes, use an image with a transparent background and swap the container colour background to match the mode (acceptable solution)">
